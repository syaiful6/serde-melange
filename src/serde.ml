...
module Ser = struct
  ...
  let serialize
        ...
        (ctx : (value, state, output) ctx)
        (ser : (value, state, output) ctx -> (output, error) result) :
    (output, error) result
    =
    ser ctx

  let serialize_sequence
        (type value state output)
  ...
  let tuple_variant
        (type value state output)
        (Ctx (_, (module S), state) as ctx : (value, state, output) ctx)
        var_type
        cstr_idx
        cstr_name
        size
  ...
  let s : type value value2 state output.
    (value, state, output) t
    -> value
...
let serialize : type value state output.
...
let deserialize : type value state.
