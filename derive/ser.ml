...
let rec serializer_for_type ~ctxt (core_type : Parsetree.core_type) =
  ...
  | Ptyp_constr (name, _args) ->
    let ser_fn =
      serializer_fn_name_for_longident name |> var ~ctxt |> Ast.pexp_ident ~loc
    in
    [%expr s [%e ser_fn]]
  | Ptyp_open (_, _)
  | Ptyp_any | Ptyp_var _
...
let gen_serialize_externally_tagged_variant_impl
      ~ctxt
      ptype_name
      type_attributes
      cstr_declarations
...
let gen_serialize_adjacently_tagged_variant_impl
      ~tag_field_name
      ~content_field_name
      ~ctxt
      ptype_name
    ...
    | Pcstr_record labels ->
      ...
      [%expr
        record ctx "" 2 (fun ctx ->
          ...
          in
          field ctx [%e Ast.estring ~loc content_field_name] (fun ctx ->
            record ctx [%e type_name] [%e field_count] (fun ctx -> [%e fields])))]
  in
...
let gen_serialize_internally_tagged_variant_impl
      ~tag_field_name:_
      ~ctxt:_
      _ptype_name
      _type_attributes
...
let gen_serialize_variant_impl
      ~ctxt
      ptype_name
      type_attributes
      cstr_declarations
  ...
  | `externally_tagged ->
    gen_serialize_externally_tagged_variant_impl
      ~ctxt
      ptype_name
      type_attributes
      cstr_declarations
  | `internally_tagged tag_field_name ->
    gen_serialize_internally_tagged_variant_impl
      ~tag_field_name
      ~ctxt
      ptype_name
      type_attributes
  ...
  | `adjacently_tagged (tag_field_name, content_field_name) ->
    gen_serialize_adjacently_tagged_variant_impl
      ~tag_field_name
      ~content_field_name
      ~ctxt
      ptype_name
