...
(* [gensym ()] creates a new symbol generator that always starts at 0. It is
   convenient for when you need to iterate and name variables several times.

   If you need a global counter that won't restart, you can rebind `gensym` to
   `gensym ()` *)
let gensym () =
...
*)
  let deserialize_with_unordered_fields ~ctxt type_attributes labels final_expr =
    ...
    (* NOTE(@leostera): Generate the placeholder values for a list of fields:
       ...
       {ocaml[ let name = ref None in let age = ref None in ... ]} *)
      ...
      List.fold_left
        (fun last (field, attr) ->
           ...
           | `with_default str ->
             [%expr
               let [%p field_pat] =
                 match ![%e field_var] with Some opt -> opt | None -> [%e str]
               in
               [%e last]])
        body
        labels
    ...
    (* NOTE(@leostera): creates the visito from strings/ints to polymorphic
       ...
       {ocaml[ Visitor.make ~visit_string:(fun _ctx str -> match str with |
       "name" -> Ok `name | "age" -> Ok `age | _ -> Error `invalid_field_type)
       ~visit_int:(fun _ctx int -> match int with | 0 -> Ok `name | 1 -> Ok `age
       | _ -> Error `invalid_field_type) () ]} *)
...
let gen_deserialize_externally_tagged_variant_impl
      ~ctxt
      ptype_name
      type_attributes
      cstr_declarations
  ...
  let deser_by_constructor _type_name idx cstr =
    ...
    | Pcstr_record labels ->
      ...
      let body =
        Record_deserializer.deserialize_with_unordered_fields
          ~ctxt
          type_attributes
          labels
        @@ fun record ->
    ...
    let cases =
      List.mapi
        (fun _idx (cstr : Parsetree.constructor_declaration) ->
           ...
           let lhs =
             Ast.ppat_constant ~loc (Ast_helper.Const.string tag_name)
           in
           let rhs =
...
let gen_deserialize_adjacently_tagged_variant_impl
      ~tag_field_name
      ~content_field_name
      ~ctxt
      ptype_name
  ...
  let deser_by_constructor _type_name idx cstr =
    ...
    | Pcstr_tuple [] ->
      let value = Ast.pexp_construct ~loc name None in
      (* there's nothing to do here because the unit variant carries no
         values *)
      [%expr Ok [%e value]]
    (* NOTE(@leostera): deserialize a newtype variant *)
    | Pcstr_tuple [ arg ] ->
      ...
      | Some `tag ->
        let rec inner_read_fields () =
          ...
          | Some `invalid_tag ->
            ...
            inner_read_fields ()
          | None ->
            (* NOTE(@sabine): we need to dispatch here because it could be a
               unit variant - in this case, there's no content field. However,
               in case of a missing content field on a non-unit variant
               constructor, we don't get a nice error message. This could be
               improved. *)
            [%e tag_dispatch]
        in
        inner_read_fields ()
      | Some `content ->
        [%e
          error_with_msg
            ~loc
            (Format.sprintf
               "field %S must appear first, found %S instead"
               tag_field_name
               content_field_name)]
        (* TODO(@sabine): Here, the Rust implementation deserializes the content
           field into an intermediate representation, then reads the tag field,
           and continues to deserialize the content field from the intermediate
           representation

           see
           https://github.com/serde-rs/serde/blob/9f8c579bf5f7478f91108c1186cd0d3f85aff29d/serde_derive/src/de.rs#L1641-L1647
           and
           https://github.com/serde-rs/serde/blob/master/serde/src/private/de.rs#L198-L207 *)
      | Some `invalid_tag ->
        let* () = ignore_any ctx in
        read_fields ctx
      | None ->
        [%e
          error_with_msg ~loc (Format.sprintf "missing field %S" tag_field_name)]]
  in
...
let gen_deserialize_internally_tagged_variant_impl
      ~tag_field_name:_
      ~ctxt:_
      _ptype_name
      _type_attributes
...
let gen_deserialize_variant_impl
      ~ctxt
      ptype_name
      type_attributes
      cstr_declarations
  ...
  | `externally_tagged ->
    gen_deserialize_externally_tagged_variant_impl
      ~ctxt
      ptype_name
      type_attributes
      cstr_declarations
  | `internally_tagged tag_field_name ->
    gen_deserialize_internally_tagged_variant_impl
      ~tag_field_name
      ~ctxt
      ptype_name
      type_attributes
  ...
  | `adjacently_tagged (tag_field_name, content_field_name) ->
    gen_deserialize_adjacently_tagged_variant_impl
      ~tag_field_name
      ~content_field_name
      ~ctxt
      ptype_name
...
*)
let gen_deserialize_record_impl
      ~ctxt
      ptype_name
      type_attributes
      label_declarations
...
let gen_deserialize_impl ~ctxt type_decl =
  ...
  let body =
    ...
    | { ptype_kind = Ptype_record label_declarations; ptype_name; _ } ->
      gen_deserialize_record_impl
        ~ctxt
        ptype_name
        type_attributes
        label_declarations
    | { ptype_kind = Ptype_variant cstrs_declaration; ptype_name; _ } ->
      gen_deserialize_variant_impl
        ~ctxt
        ptype_name
        type_attributes
        cstrs_declaration
